...
namespace ArcaneEngine
{
    ...
    public sealed class PlayerController : MonoBehaviour, IDamageable, ITargetable
    {
        ...
        private void Update()
        {
            ...
            if (runActive && TickRunResources(world))
                return;

            if (world.ModalOpen || RunStartScreen.IsOpen)
            {
                DecelerateMovement(Time.unscaledDeltaTime);
                return;
            }

            if (runActive && Time.time < _stunUntil)
            {
                DecelerateMovement(Time.deltaTime);
                return;
            }

            // Sanctuary movement is valid; combat simulation remains run-only.
            Move();
...
private void LateUpdate()
        ...
        {
            ControlSettings controls = ProfileManager.Current.controls;
            float x = (ArcaneInput.GetKey(controls.moveRight) ? 1f : 0f) -
                      (ArcaneInput.GetKey(controls.moveLeft) ? 1f : 0f);
            float z = (ArcaneInput.GetKey(controls.moveForward) ? 1f : 0f) -
                      (ArcaneInput.GetKey(controls.moveBack) ? 1f : 0f);
            ...
            if (rig != null && !ProfileManager.Current.accessibility.worldRelativeMovement)
                movement = rig.PlanarRight * x + rig.PlanarForward * z;

            if (movement.sqrMagnitude > 1f)
                movement.Normalize();

            bool combatConditionsApply = GameWorld.Instance != null && GameWorld.Instance.RunActive;
            float conditionMultiplier = !combatConditionsApply
                ? 1f
            ...
            transform.position = DungeonObstacle.Resolve(
                ClampToRoom(transform.position + _moveVelocity * Time.deltaTime));
        }
