...
namespace ArcaneEngine
{
    ...
    public static class ReactionLineageRegistry22
    {
        ...
        private static readonly Dictionary<long, Dictionary<EntityId, float>> TargetTimes =
            new Dictionary<long, Dictionary<EntityId, float>>();
            ...
            float immunitySeconds)
        {
            ...
            Dictionary<EntityId, float> targets;
            if (!TargetTimes.TryGetValue(context.originCastId, out targets))
            {
                targets = new Dictionary<EntityId, float>();
