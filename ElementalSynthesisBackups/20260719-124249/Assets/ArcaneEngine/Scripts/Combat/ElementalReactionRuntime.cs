...
namespace ArcaneEngine
{
    ...
    public sealed class ElementalReactionController : MonoBehaviour
    {
        ...
        private void ResolvePendingReaction()
        {
            ...
            if (CreatesResidue(definition.signature) ||
                definition.ElementCount >= 4)
            {
                ElementalReactionRuntime.SpawnField(
                    definition.signature,
                    transform.position,
                    definition.radius * 0.72f,
                    definition.duration,
                    burstDamage * 0.12f,
                    definition.spreadStacks * 0.24f);
            }
        ...
        public void ResolveDeath()
        {
            ...
            float damage =
                ...
                Mathf.Clamp(
                    0.025f +
                    definition.ElementCount * 0.011f,
                    0.04f,
                    0.14f) *
            ...
            ElementalReactionRuntime.SpawnField(
                ...
                transform.position,
                definition.radius,
                definition.duration * 1.25f,
                damage * 0.14f,
                definition.spreadStacks * 0.38f);

            if (GameWorld.Instance != null)
            {
                GameWorld.Instance.Log(
                    definition.displayName +
                    " death reaction erupts from " +
                    _owner.DisplayName +
                    ".");
            }
        }
